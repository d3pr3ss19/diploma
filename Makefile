.PHONY: help db-setup smoke smoke-full smoke-backend smoke-frontend test-frontend e2e-api e2e-rbac e2e-auth-integration ci-auth-check ci-prepare-auth-db verify-scripts verify-structure verify-configs verify-token-helper verify-wait-for-http verify-prisma-migrate verify-prisma-redeploy verify-auth-seed verify-auth-seed-scope verify-auth-seed-after-runtime verify-auth-runtime verify-auth-runtime-domain-safety verify-auth-refresh-sessions verify-auth-multi-login verify-auth-logout-idempotent verify-auth-repeat-logout verify-auth-logout-noop-tokens verify-auth-logout-empty-token verify-auth-logout-nonstring-token verify-auth-logout-missing-token verify-auth-logout-extra-fields verify-auth-external-db-url verify-auth-no-seed verify-tooling
...
help:
	...
	@echo "  make verify-auth-logout-empty-token - logout с пустым refreshToken не ломает сессии и возвращает контролируемый ответ"
	@echo "  make verify-auth-logout-nonstring-token - logout(number/null refreshToken) возвращает validation error и не меняет БД"
	@echo "  make verify-auth-logout-missing-token - logout без refreshToken возвращает validation error и не меняет БД"
	@echo "  make verify-auth-logout-extra-fields - logout с лишними полями payload возвращает 400 и не меняет БД"
...
verify-auth-logout-empty-token:
	./scripts/verify-auth-logout-empty-token.sh

verify-auth-logout-nonstring-token:
	./scripts/verify-auth-logout-nonstring-token.sh

verify-auth-logout-missing-token:
	./scripts/verify-auth-logout-missing-token.sh

verify-auth-logout-extra-fields:
	./scripts/verify-auth-logout-extra-fields.sh
