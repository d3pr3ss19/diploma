...
1. Подними PostgreSQL и создай локальную БД/пользователя для проекта ("нормальная" БД вместо in-memory):

```bash
make db-setup
```

> По умолчанию создаются: `diploma_user` / `diploma` на `localhost:5432`.
> При необходимости можно переопределить `APP_DB_*` и `POSTGRES_ADMIN_*` переменные окружения.
...
3. В отдельном терминале подними frontend:
...
4. Открой `http://localhost:5173/login`.

На текущем этапе авторизация использует пользователей из PostgreSQL (через Prisma).
Подробный гайд входа/авторизации: `docs/site-access-guide.md`.
...
- `docs/site-access-guide.md` — быстрый вход на сайт, логин и добавление пользователя
...
- ✅ Шаг 8.1: добавлен релизный чек-лист предзащиты (`docs/release-checklist.md`).
- ✅ Шаг 8.2: добавлен компактный demo-сценарий защиты (`docs/demo-defense-scenario.md`).


## До финала проекта

Ориентир на текущий момент: остался **1 укрупнённый этап**:

1. Шаг 8 — предзащита/финальная упаковка проекта.

## Мониторинг прогресса

- Главный трекер: `docs/progress.md`
- Release checklist (Шаг 8): `docs/release-checklist.md`
- Demo сценарий защиты (Шаг 8): `docs/demo-defense-scenario.md`
...
- Make targets: `make smoke`, `make smoke-full`, `make smoke-backend`, `make smoke-frontend`, `make e2e-api`, `make e2e-rbac`, `make e2e-auth-integration`, `make ci-auth-check`, `make ci-prepare-auth-db`, `make verify-scripts`, `make verify-structure`, `make verify-configs`, `make verify-token-helper`, `make verify-wait-for-http`, `make verify-prisma-migrate`, `make verify-prisma-redeploy`, `make verify-auth-seed`, `make verify-auth-seed-scope`, `make verify-auth-seed-after-runtime`, `make verify-auth-runtime`, `make verify-auth-runtime-domain-safety`, `make verify-auth-refresh-sessions`, `make verify-auth-multi-login`, `make verify-auth-logout-idempotent`, `make verify-auth-repeat-logout`, `make verify-auth-logout-noop-tokens`, `make verify-auth-logout-empty-token`, `make verify-auth-logout-nonstring-token`, `make verify-auth-logout-missing-token`, `make verify-auth-logout-extra-fields`, `make verify-auth-external-db-url`, `make verify-auth-no-seed`, `make verify-tooling`, `make test-frontend`
...
- Repeat logout check: `make verify-auth-repeat-logout` (проверяет, что второй `logout` тем же refresh token остаётся успешным и не меняет `refresh_sessions` сверх первого revoke).
- Logout no-op token check: `make verify-auth-logout-noop-tokens` (проверяет, что `logout` с access token и malformed token остаётся безопасным no-op и не затрагивает `refresh_sessions`).
- Logout empty-token check: `make verify-auth-logout-empty-token` (проверяет контролируемый ответ `logout` с пустым `refreshToken` и отсутствие изменений в `refresh_sessions`).
- Logout non-string token check: `make verify-auth-logout-nonstring-token` (проверяет, что `logout` с нестроковым `refreshToken` (`number/null`) возвращает валидационный `400` и не меняет `refresh_sessions`).
- Logout missing-token check: `make verify-auth-logout-missing-token` (проверяет, что `logout` без поля `refreshToken` возвращает валидационный `400` и не меняет `refresh_sessions`).
- Logout extra-fields check: `make verify-auth-logout-extra-fields` (проверяет, что `logout` с лишними полями payload отклоняется с `400` при `forbidNonWhitelisted` и не меняет `refresh_sessions`).
