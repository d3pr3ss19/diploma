...
Перейти к Шагу 7.50: добавить verify-check, что `logout` c нестроковым `refreshToken` (например, number/null) возвращает валидационную ошибку и не меняет `refresh_sessions`.
