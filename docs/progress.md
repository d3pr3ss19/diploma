...
Перейти к Шагу 7.47: добавить verify-check для повторного logout по одному и тому же refresh token, чтобы второй вызов оставался успешным и не менял состояние БД сверх первого revoke.
