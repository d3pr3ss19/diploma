...
fail() {
  echo "[verify-auth-logout-missing] ❌ $1" >&2
  exit 1
}
...
TEMP_PG_LABEL="verify-auth-logout-missing"
TEMP_PG_DB_NAME="${VERIFY_AUTH_LOGOUT_MISSING_DB_NAME:-diploma_verify_logout_missing}"
TEMP_PG_DB_USER="${VERIFY_AUTH_LOGOUT_MISSING_DB_USER:-postgres}"
TEMP_PG_DB_PASSWORD="${VERIFY_AUTH_LOGOUT_MISSING_DB_PASSWORD:-postgres}"
TEMP_PG_DB_IMAGE="${VERIFY_AUTH_LOGOUT_MISSING_DB_IMAGE:-postgres:16-alpine}"
TEMP_PG_DB_PORT="${VERIFY_AUTH_LOGOUT_MISSING_DB_PORT:-}"
TEMP_PG_DATABASE_URL="${TEST_DATABASE_URL:-}"
BACKEND_PORT="${VERIFY_AUTH_LOGOUT_MISSING_PORT:-3950}"
BASE_URL="${BASE_URL:-http://127.0.0.1:${BACKEND_PORT}/api/v1}"
AUTH_JWT_SECRET="${AUTH_JWT_SECRET:-verify-auth-logout-missing-secret}"
WAIT_TIMEOUT="${WAIT_TIMEOUT:-60}"
WAIT_INTERVAL="${WAIT_INTERVAL:-1}"
LOGIN_EMAIL="${VERIFY_AUTH_LOGOUT_MISSING_EMAIL:-operator@kp.local}"
LOGIN_PASSWORD="${VERIFY_AUTH_LOGOUT_MISSING_PASSWORD:-password123}"

TMP_DIR="$(mktemp -d)"
BACKEND_LOG="$TMP_DIR/backend.log"
LOGIN_RESPONSE="$TMP_DIR/login.json"
MISSING_LOGOUT_RESPONSE="$TMP_DIR/logout-missing.json"

temp_pg_setup

[[ -n "${TEMP_PG_CONTAINER_ID:-}" ]] || fail "SQL-проверка logout missing-token требует временную container-БД; внешний TEST_DATABASE_URL не поддерживается"

run_backend_step() {
  local title="$1"
  shift
  echo "[verify-auth-logout-missing] ▶ $title"
  (
    cd "$BACKEND_DIR"
...
BACKEND_RUNTIME_LABEL="verify-auth-logout-missing"
...
echo "[verify-auth-logout-missing] ✅ создана одна активная refresh_session"

MISSING_LOGOUT_CODE="$(curl -sS -o "$MISSING_LOGOUT_RESPONSE" -w "%{http_code}" \
  -X POST "$BASE_URL/auth/logout" \
  -H 'Content-Type: application/json' \
  -d '{}')"
[[ "$MISSING_LOGOUT_CODE" == "400" ]] || fail "logout(без refreshToken) должен вернуть 400, получен $MISSING_LOGOUT_CODE"
grep -q '"statusCode"[[:space:]]*:[[:space:]]*400' "$MISSING_LOGOUT_RESPONSE" || fail "logout(без refreshToken) не содержит statusCode=400"
echo "[verify-auth-logout-missing] ✅ payload без refreshToken отклоняется валидацией (HTTP 400)"

assert_scalar_equals "SELECT COUNT(*) FROM refresh_sessions WHERE user_id = '$USER_ID';" "1"
assert_scalar_equals "SELECT COUNT(*) FROM refresh_sessions WHERE user_id = '$USER_ID' AND revoked_at IS NULL;" "1"
assert_scalar_equals "SELECT COUNT(*) FROM refresh_sessions WHERE user_id = '$USER_ID' AND token_hash = '$REFRESH_HASH' AND revoked_at IS NULL;" "1"
echo "[verify-auth-logout-missing] ✅ валидационная ошибка не меняет refresh_sessions"

VALID_REFRESH_CODE="$(curl -sS -o /dev/null -w "%{http_code}" \
  -X POST "$BASE_URL/auth/refresh" \
  -H 'Content-Type: application/json' \
  -d "{\"refreshToken\":\"$REFRESH_TOKEN\"}")"
[[ "$VALID_REFRESH_CODE" == "200" || "$VALID_REFRESH_CODE" == "201" ]] || fail "ожидался успешный refresh после logout(без refreshToken), получен $VALID_REFRESH_CODE"
echo "[verify-auth-logout-missing] ✅ исходная активная сессия остаётся рабочей"

backend_runtime_ensure_alive

echo "[verify-auth-logout-missing] 🎉 Logout(missing refreshToken) verify-check пройден"
