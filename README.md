...
## Бизнес-сценарий абонента (добавлено)
1. Абонент выбирает свой лицевой счёт и создаёт заявку.
2. В заявке фиксируются: категория, приоритет, контактный телефон и желаемое время визита.
3. После создания абонент видит историю обработки заявки (статусы и комментарии оператора).
4. Оператор берёт заявку в работу, назначает исполнителя и оставляет комментарии по ходу выполнения.
