...
Перейти к Шагу 7.49: добавить verify-check устойчивости `logout` к payload с пустым `refreshToken`, чтобы ответ оставался контролируемым и состояние сессий не портилось.
