...
- [x] Шаг 8.5: для admin добавлено редактирование карточки абонента и улучшен UX создания заявок (выбор абонента → выбор accountId), а `createdByUserId` теперь берётся с backend из авторизованного токена.
- [x] Шаг 8.6: локализация под РФ (телефоны +7, рубли в dashboard/account), плюс admin-функция деактивации пользователя и выравнивание кнопок фильтров на ключевых страницах.
...
Перейти к Шагу 8.7: провести визуальный smoke по страницам Dashboard/Абоненты/Заявки и зафиксировать UX-правки по кнопкам/формам перед финальным dry-run.
